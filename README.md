...
현재는 사용자가 직접 필터를 선택해서 기사를 찾는 구조다. 다음 단계는 자연어로 질문하면 DB에 쌓인 기사 데이터를 기반으로 에이전트가 직접 답해주는 기능이다.

```
"오늘 핫한 트렌드 기사들 모아줘"
→ 최근 24시간 keywords 빈도 집계 → 상위 기사 목록 반환

"오늘 경제 뉴스 한 줄 요약해줘"
→ 경제 카테고리 오늘 기사 summary 수집 → LLM으로 통합 브리핑 생성

"삼성 관련 기사 있어?"
→ keywords/summary 검색 → 관련 기사 목록 반환
```

DB에 이미 카테고리·핵심어·요약이 정형화되어 저장되어 있기 때문에, 에이전트가 자연어 질의를 의도 분류 → DB 쿼리 → LLM 답변 생성 흐름으로 처리하기에 적합한 구조다.

### 개인화 추천

- Supabase Auth 로그인
- 사용자별 열람 기록·카테고리 선호도 저장
- t-SNE 유사도 추천과 결합한 하이브리드 추천
